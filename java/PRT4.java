...
/*
    This test case contains 2 threads and 4 operations in each thread
    The first thread contains Insert, SelectSpecific, Update, SelectSpecific
    The second thread contains Insert, SelectAll, DeleteSpecific, SelectAll
    The update in 1st thread happens on the inserted rows of 2nd transaction while the 
    delete in 2nd thread happens on the inserted rows of 1st transaction
*/

public class PRT4 {                    // Update vs Delete
    ...
    public static void random_operations(int[] list,int tid)
    {
        ...
        if(tid==0)
        {
            list[1] = 4;         
            list[2] = 1;                    // Insert
            list[3] = 5;                    // SelectSpecific
            list[4] = 2;                    // Update
            list[5] = 5;                    // SelectSpecific
        }
        else
        if(tid==1)
        {
...
public static void main(String[] args) throws SQLException {
    ...
    try{
        Iteration.fout = new FileWriter("../text/"+Iteration.name+"_PRT4_iterations.txt");
    }catch(IOException e){
    ...
    try{
        fout = new FileWriter("../text/"+Iteration.name+"_PRT4_summary.txt");
        int k=0;
        for(int i=0;i<NUM_THREADS;i++)
        {
